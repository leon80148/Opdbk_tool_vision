...
| `CO02F`      | 檢查報告，儲存各項檢查的詳細報告內容。           | `kcstmr`, `fdate`                   |
| `CO03M`      | 檢驗單/帳務摘要，記錄就醫批次的摘要、診斷與帳務金額。   | `kcstmr`, `idate`, `itime`          |
| `CO03L`      | 看診紀錄，記錄病患實際看診的診斷與卡序資訊。      | `kcstmr`, `date`, `time`            |
| `co05b`      | 預約紀錄，儲存病患預約掛號資訊。              | `kcstmr`, `tbkdt`, `tsts`, `tartime` |
| `CO05O`      | 掛號紀錄，記錄病患掛號的詳細時間與看診狀態。      | `kcstmr`, `tbkdate`, `tbktime`      |
...
**用途**：
判斷是否為慢性病患（看開藥天數）、查詢疫苗施打紀錄、查詢超音波/內視鏡醫令、慢性病管理追蹤。
...
| 代碼       | 名稱           | 類別    | 執行間隔（健保規定） |
| -------- | ------------ | ----- | ---------- |
| `P1407C` | 糖尿病-初期      | 糖尿病   | 50 天      |
| `P1408C` | 糖尿病-穩定期     | 糖尿病   | 71 天      |
| `P1409C` | 糖尿病-控制不良    | 糖尿病   | 71 天      |
| `P7001C` | 糖尿病-併發症     | 糖尿病   | 71 天      |
| `P7002C` | 糖尿病-其他      | 糖尿病   | 71 天      |
| `P4301C` | 腎臟病-初期      | 腎臟病   | 77 天      |
| `P4302C` | 腎臟病-穩定期     | 腎臟病   | 161 天     |
| `P7501C` | 代謝症候群-初期    | 代謝症候群 | 71 天      |
| `P7502C` | 代謝症候群-穩定期   | 代謝症候群 | 71 天      |
| `P7503C` | 代謝症候群-控制不良  | 代謝症候群 | 71 天      |
| `19001C` | 腹部超音波（健保）   | 影像檢查  | 6 個月      |
| `19009C` | 腹部超音波（自費）   | 影像檢查  | 6 個月      |
| `19012C` | 甲狀腺超音波      | 影像檢查  | 6 個月      |
| `15021C` | 細針穿刺        | 侵入性檢查 | 1 年       |
| `17003C` | 肺功能檢查（簡易）   | 功能檢查  | 6 個月      |
| `17006C` | 肺功能檢查（完整）   | 功能檢查  | 6 個月      |
| `21004C` | 尿流速檢查       | 功能檢查  | 1 年       |

---

### 2.3 `CO02F` - 檢查報告 (Examination Reports)

**用途**：
儲存各項檢查的詳細報告內容，如超音波、內視鏡、X光等檢查結果的文字描述。

| 欄位名稱     | 類型      | Nullable | 描述 / 備註        |
| -------- | ------- | -------- | -------------- |
| `kcstmr` | varchar | No       | **複合主鍵**，病歷號   |
| `fdate`  | varchar | No       | **複合主鍵**，執行日期  |
| `ftext`  | text    | Yes      | 報告內容（檢查結果文字描述） |

---

### 2.4 `CO03M` - 檢驗單批次與帳務摘要 (Billing & Visits)
...
### 2.5 `CO03L` - 看診紀錄 (Visit Records)

**用途**：
記錄病患實際看診的診斷、卡序與就診類別資訊。

| 欄位名稱     | 類型      | Nullable | 描述 / 備註                      |
| -------- | ------- | -------- | ---------------------------- |
| `kcstmr` | varchar | No       | **複合主鍵**，病歷號                 |
| `date`   | varchar | No       | **複合主鍵**，看診日期                |
| `time`   | varchar | No       | **複合主鍵**，看診時間                |
| `lisrs`  | varchar | Yes      | 卡序（健保申報類別）                   |
| `labdt`  | varchar | Yes      | 診斷代碼                         |
| `labeno` | varchar | Yes      | 註記                           |
| `lcs`    | varchar | Yes      | 就診類別（門診類型，如：一般門診、慢性病、急診等） |
| `lldtt`  | varchar | Yes      | 慢簽次數（慢性病連續處方箋次數）             |
| `dayqty` | varchar | Yes      | 開藥天數（處方用藥天數）                 |

---

### 2.6 `co05b` - 預約紀錄 (Appointment Records)
...
### 2.7 `CO05O` - 掛號紀錄 (Registration Records)

**用途**：
記錄病患掛號的詳細時間、看診狀態與身分別，用於追蹤掛號到完診的流程。
...
| `tisrs`    | varchar | Yes      | 卡序（健保申報類別）                   |
| `lm`       | varchar | Yes      | 身分別（如：健保、自費、榮民等）             |

---

### 2.8 `CO18H` - 檢驗項目歷史結果檔 (Lab History)
...
### 2.9 `VISHFAM` - 家戶名單 (Householder List)
